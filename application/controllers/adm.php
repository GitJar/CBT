...
class Adm extends CI_Controller
{
	...
	public function h_ujian()
	{
		...
		if ($uri3 == "det") {
			...
			$q_datanya = $this->db->query("
	        	SELECT a.id, b.nama, a.nilai, a.jml_benar, a.jml_salah, a.nilai_bobot
				FROM tr_ikut_ujian a
				INNER JOIN m_siswa b ON a.id_user = b.id
				WHERE a.id_tes = '$uri4' 
			...
			foreach ($q_datanya as $d) {
				...
				$data_ok[7] = '<a href="' . base_url() . 'adm/h_ujian/batal_selesai/' . $d['id'] . '/' . $this->uri->segment(4) . '" class="btn btn-warning btn-xs" onclick="return confirm(\'Anda yakin...?\');"><i class="glyphicon glyphicon-remove" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Tambah Waktu</a>';
				$data[] = $data_ok;
			}
			$json_data = array(
				"draw" => $draw,
		...
		} else if ($uri3 == "batal_selesai") {
			$tambah_waktu = $newDate = date('Y-m-d H:i:s', strtotime('+10 minutes'));
			$this->db->query("UPDATE tr_ikut_ujian SET status='Y' WHERE id = '$uri4'");
			$this->db->query("UPDATE tr_ikut_ujian SET tgl_selesai='$tambah_waktu' WHERE id = '$uri4'");
			redirect('adm/h_ujian/det/' . $uri5);
		}else if ($uri3 == "data") {
			$start = $this->input->post('start');
			$length = $this->input->post('length');
	...
	public function hasil_ujian_cetak()
	{
		...
		$a['hasil'] = $this->db->query("SELECT m_siswa.nama, tr_ikut_ujian.nilai, tr_ikut_ujian.jml_benar, tr_ikut_ujian.jml_salah, tr_ikut_ujian.nilai_bobot
										FROM tr_ikut_ujian
	...
	public function ikut_ujian()
	{
		...
		if ($uri3 == "simpan_satu") {
			...
			$this->db->query("UPDATE tr_ikut_ujian SET jml_benar = " . $jumlah_benar .  ", jml_salah = " . $jumlah_salah .  ", nilai = " . $nilai . ", nilai_bobot = " . $nilai_bobot . ", status = 'N' WHERE id_tes = '$id_tes' AND id_user = '" . $a['sess_konid'] . "'");
			$a['status'] = "ok";
			j($a);
			exit;
		} else if ($uri3 == "token") {
	...
	public function sudah_selesai_ujian()
	{
		...
		$q_nilai = $this->db->query("SELECT nilai, tgl_mulai, tgl_selesai FROM tr_ikut_ujian WHERE id_tes = $uri3 AND id_user = '" . $a['sess_konid'] . "' AND status = 'N'")->row();
		if (empty($q_nilai)) {
			...
			if ($this->config->item('tampil_nilai')) {
				$a['data'] = "<div class='alert alert-danger'>Anda telah mengikuti ujian ini pada : <strong style='font-size: 16px'>" . tjs($q_nilai->tgl_mulai, "l") . "</strong>, dan mendapatkan nilai : <strong style='font-size: 16px'>" . $q_nilai->nilai . "</strong></div>";
			} else {
				$a['data'] = "<div class='alert alert-danger'>Anda telah mengikuti ujian ini pada : <strong style='font-size: 16px'>" . tjs($q_nilai->tgl_mulai, "l") . "</strong>. Nilai akan diumumkan oleh Guru yang bersangkutan.</div>";
			}
		}
		$this->load->view('aaa', $a);
	}
